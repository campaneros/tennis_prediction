#!/usr/bin/env python3
"""
Allena un LSTM 2 layer per predire la probabilità che il server vinca il punto.
Esporta un CSV punto-per-punto con p_server_point_prob.
"""
...
def _load_sequences(path: str) -> Tuple[List[Dict], List[str]]:
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"{path} non trovato. Esegui prima prepare_wimbledon_server_point_sequences.py"
        )
...
def export_point_probs(model, sequences: List[Dict], feature_names: List[str], out_csv: str, device: str):
                ...
                rows.append(
                    {
                        ...
                        "PointServer": server,
                        "server_wins_point": int(label == 1),
                        "p_server_point_prob": float(prob),
                    }
                )
    import pandas as pd
    ...
    print(f"Salvato CSV punto-per-punto (server target): {out_csv} (righe: {len(df)})")


def main():
    parser = argparse.ArgumentParser(description="Train LSTM punto-per-punto (server vince il punto).")
    parser.add_argument("--data", default="data/wimbledon_server_point_sequences.pkl", help="Pickle da prepare_wimbledon_server_point_sequences.py")
    ...
    parser.add_argument("--export-csv", default="data/lstm_server_point_probs.csv", help="CSV output punto-per-punto.")
    ...
    torch.save({"model_state": model.state_dict(), "feature_names": feature_names}, "models/lstm_server_point_model.pt")
    print("Modello salvato in models/lstm_server_point_model.pt")
