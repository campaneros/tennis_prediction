#!/usr/bin/env python3
"""
Prepara sequenze punto-per-punto con target = il server vince il punto.
Filtri anno/genere come gli altri script, salva un pickle con feature e label.
...
def _load_points(min_year: int = 2019, max_year: int = None, gender: str = "both") -> pd.DataFrame:
    ...
    # Pulizia NaN sulle colonne necessarie
    numeric_cols = [
        "P1GamesWon",
        "P2GamesWon",
        "P1BreakPoint",
...
def _build_sequences(df: pd.DataFrame) -> Tuple[List[Dict], List[str]]:
    ...
    for match_id, g in df.groupby("match_id", sort=False):
        g = g.sort_values(["SetNo", "GameNo", "PointNumber"])
        feats = []
        labels = []
        meta_rows = []
        for _, row in g.iterrows():
            server = int(row["PointServer"])
            ...
            labels.append(int(row["PointWinner"] == server))  # 1 se il server vince il punto
            meta_rows.append(
                (match_id, int(row["SetNo"]), int(row["GameNo"]), int(row["PointNumber"]), server)
            )
        if feats:
            sequences.append(
                {
...
def prepare_sequences(
    out_path: str = "data/wimbledon_server_point_sequences.pkl",
    min_year: int = 2019,
    max_year: int = None,
    gender: str = "both",
) -> str:
    ...
    payload = {
        ...
        "info": {
            "source": "wimbledon points (server wins point)",
    ...
    print(f"Salvato {len(sequences)} sequenze punto-per-punto (server target) in {out_path}")
    return out_path
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Prepara sequenze punto-per-punto con target server vince il punto.")
    parser.add_argument("--out", default="data/wimbledon_server_point_sequences.pkl", help="Percorso output pickle.")
