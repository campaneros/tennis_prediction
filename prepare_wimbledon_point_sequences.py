#!/usr/bin/env python3
"""
Prepara sequenze punto-per-punto per un LSTM che predice chi vince il punto.
Filtra per anno/genere e salva un pickle con feature e label per ogni punto
ordinati nel match. Non tocca file esistenti.
"""
...
def _build_sequences(df: pd.DataFrame) -> Tuple[List[Dict], List[str]]:
    ...
    for match_id, g in df.groupby("match_id", sort=False):
        ...
        for _, row in g.iterrows():
            ...
            labels.append(int(row["PointWinner"] == 1))  # 1 se P1 vince il punto
            meta_rows.append(
...
def prepare_sequences(
    out_path: str = "data/wimbledon_point_sequences.pkl",
    min_year: int = 2019,
    max_year: int = None,
    gender: str = "both",
) -> str:
    ...
    payload = {
        ...
        "info": {
            "source": "wimbledon points (point-by-point)",
    ...
    print(f"Salvato {len(sequences)} sequenze punto-per-punto in {out_path}")
    return out_path
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Prepara sequenze punto-per-punto (filtri anno/genere).")
    parser.add_argument("--out", default="data/wimbledon_point_sequences.pkl", help="Percorso output pickle.")
